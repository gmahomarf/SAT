<?php
    ...
    $script = 
        "$(window).addEvent('domready', function() {\n
            ...
            \n
            var frmVal = new Form.Validator.Inline('CorreoAlertaAgregarForm', {\n
    ...
    echo $html->tag('h1', 'Agregar Correo para Alerta');
    echo $html->tag('fieldset');
    echo $html->tag('legend', 'Agregar Correo para Alerta');
    echo $form->create('CorreoAlerta', array('url' => 'agregar/'. $alertaId, 'id' => 'CorreoAlertaAgregarForm'));
        echo $form->input('alerta_id', array('type' => 'hidden', 'value' => $alertaId));
        echo $form->input('direccion', array('class' => 'required'));
